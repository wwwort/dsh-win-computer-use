...
同一件事既可以由主代理自己做，也可以**派一个子代理**去做。省不省，**完全取决于主代理本来要几轮**——
这一点用同一台机器、同一个任务、两边的会话日志对照量过：

| 同一任务 | 主代理每轮上下文 | 子代理每轮上下文 |
|---|---|---|
| 实测 | **522,925 token** | **19,818 token**（5 轮：18,457 → 21,226） |

| 做法 | 代价 |
|---|---|
| 主代理直接做（一次 `computer` 批量调用） | **522,925** —— 1 轮 |
| 派子代理做 | 525,007（1 轮）+ **99,092**（子代理 5 轮）= **624,099** |

**这件事派子代理反而贵 19%。** 因为它一次能写全，主代理一轮就做完了，而新起的子代理要五轮。
派子代理真正占优的场景，是主代理本来需要**更多轮**的时候，而那时差距不是一点点：

```
N × 522,925  >  525,007 + K × 19,818        N = 主代理轮数，K = 子代理轮数
⇒ N ≥ 2 就划算（K 在 26 轮以内都成立）
```

所以规则看的是**轮数**，不是「这是不是一个电脑操作」：

- **不看屏幕就能把整个 `steps` 写全** → 直接 `computer` 一次调用。
- **下一步取决于屏幕上读到什么（本来要 ≥2 轮）** → 派子代理。
- `subagent` 要传 `run_in_background: false`，这样「派发 + 收结果」算**同一轮**；后台派发要多一轮收结果，门槛抬到 N ≥ 3。
- **前台子代理不保留**（事后 `list_agents` 报 none），没法复用；后台子代理是 durable 的，可以用 `send_message` 接着下指令，代价就是那一轮。**要省轮次还是要可复用，只能选一个。**

另一半理由不是 token 数字：子代理读到的所有东西 —— UIA 树、截图、失败的尝试 —— 都留在它自己那里。
主上下文里吸收进去的任何东西都不免费，因为它会在**之后每一轮**被重发，直到会话结束。
