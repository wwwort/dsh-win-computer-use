...
const STEP_ITEM = {
  ...
  properties: {
    ...
    x: { type: 'integer' as const, description: 'Screen x in physical pixels (click / windows move). For type|key it is the point to CLICK FIRST so the control really has keyboard focus -- activating a window does not focus its input box.' },
    y: { type: 'integer' as const, description: 'Screen y in physical pixels (click / windows move). See x: for type|key this is the focus click.' },
    via: { type: 'string' as const, enum: ['keys', 'clipboard'], description: 'type only, physical: how to deliver the text. "clipboard" writes it to the clipboard and presses Ctrl+V -- far faster for long text or CJK. It saves and restores the previous clipboard text; a non-text clipboard cannot be restored.' },
    verify: { type: 'boolean' as const, description: 'type only, physical: after typing, read the window text back and report whether the text landed. Best effort -- contenteditable fields update their accessibility text lazily.' },
    verify_delay_ms: { type: 'integer' as const, description: 'type only: settle before the verification read (default 500).' },
    ...
    state: { type: 'string' as const, enum: ['exists', 'gone', 'foreground', 'visible', 'text_stable'], description: 'wait only, default exists. text_stable = return once the window text stops changing (and return it as text) -- use it to wait for a reply instead of guessing a sleep. Add absent/contains when a "still working" placeholder could sit still long enough to look finished.' },
    absent: { type: 'string' as const, description: 'wait state:"text_stable" only: the settled text must NOT contain this. Guards against settling on a still-working placeholder (e.g. absent:"正在回应" on the ChatGPT desktop app).' },
    contains: { type: 'string' as const, description: 'wait state:"text_stable" only: the settled text must contain this.' },
    tail: { type: 'integer' as const, description: 'read/wait only: return just the last N characters. The newest content of a chat, status line or log sits at the end of the document text.' },
    max_chars: { type: 'integer' as const, description: 'read/wait only: hard cap on returned characters (default 4000, keeps the tail).' },
    from: { type: 'string' as const, enum: ['tail', 'head'], description: 'read only: which end to keep when the text is truncated (default tail).' },
...
const STEPS_DESCRIPTION = [
  ...
  '• type：ref 或 name…，加 text。元素树里找不到输入框时（Chromium 常见），用 mode:"physical" + x/y：引擎会**先点那个坐标拿键盘焦点**再打真实按键（长文本可用 via:"clipboard" 走剪贴板粘贴）。',
  '• key：keys:"ctrl+s"。"enter"、"alt+F4" 这类组合键只能走真实键盘。**要送进某个输入框就先给 x/y** —— 光激活窗口不会把键盘焦点给它，Enter/Ctrl+A 会落到应用自己当前的焦点上（这就是"发送了却没发出去"的成因）。',
...
const OPERATOR_SKILL = {
  ...
  content: [
    ...
    '',
    '## 聊天/讨论循环配方（一轮一条调用，全程不碰元素树）',
    '',
    '```jsonc',
    '// 给 x/y = 让引擎先点一下输入框拿到键盘焦点，再打真实按键',
    '{"steps":[',
    '  {"op":"type","window":"ChatGPT","mode":"physical","x":1436,"y":1205,"text":"<你的问题>","verify":true},',
    '  {"op":"key","keys":"enter","window":"ChatGPT"},',
    '  {"op":"wait","window":"ChatGPT","state":"text_stable","stable_ms":2500,"timeout_ms":90000,',
    '   "absent":"正在回应","contains":"ChatGPT 说","tail":2500}',
    ']}',
    '```',
    '',
    '五条实测教训（每条都真的让我白跑过一轮）：',
    '1. **`focus` 只激活窗口，不给输入框键盘焦点。** 光激活就打字会被丢掉；`ctrl+a` 选不到东西；**Enter 会落到应用自己当前的焦点上 —— "发送了却没发出去"就是这么来的**。所以 type 要给 `x/y`（或控件 ref），让引擎先点一下。',
    '2. **同一批 steps 内不要归还前台。** 引擎已改成：批内只还原光标，**前台在整次调用结束时统一还给用户**（结果是 `foregroundRestored`）。早期版本每步都还前台，于是 `type` 之后输入框丢焦点、下一步的 Enter 无处可落。（要"焦点留给某个应用"，给那一步 `restore:false`。）',
    '3. **纯"文本不再变化"会在占位状态上误判。** 实测 `wait state:"text_stable"` 曾在「ChatGPT 正在回应」上判定稳定并返回了一个**还没到的回复**。用 `absent:"正在回应"` + `contains:"ChatGPT 说"` 这类护栏钉死完成条件。',
    '4. **元素树会塌。** 实测 ChatGPT 窗口的 UIA 控件树只剩 13 个节点（标题栏按钮 + 空 Pane），网页内容全消失 —— 而同一时刻 `read` 照常拿到 25,722 字符。所以浏览器类应用里：**写走 `x/y` + 物理按键，读走 `read` 文本**，整条链路都不需要元素树。',
    '5. **contenteditable 的文本是懒更新的。** 写完立刻 `read` 可能读回旧值（我因此**误判过一次"输入失败"，其实成功了**）。用 `verify:true` 让引擎自己隔一会儿读回，或直接用 `wait state:"text_stable"`。',
    '',
    '长文本（几百字以上）用 `"via":"clipboard"` —— 一次剪贴板写入 + Ctrl+V，比逐字符 SendInput 快几个数量级；它会保存并还原你原来的剪贴板文本（**非文本内容无法还原**）。',
    '',
