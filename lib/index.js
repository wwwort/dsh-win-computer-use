...
const STEP_ITEM = {
    ...
    properties: {
        ...
        x: { type: 'integer', description: 'Screen x in physical pixels (click / windows move). For type|key it is the point to CLICK FIRST so the control really has keyboard focus -- activating a window does not focus its input box.' },
        y: { type: 'integer', description: 'Screen y in physical pixels (click / windows move). See x: for type|key this is the focus click.' },
        via: { type: 'string', enum: ['keys', 'clipboard'], description: 'type only, physical: how to deliver the text. "clipboard" writes it to the clipboard and presses Ctrl+V -- far faster for long text or CJK. It saves and restores the previous clipboard text; a non-text clipboard cannot be restored.' },
        verify: { type: 'boolean', description: 'type only, physical: after typing, read the window text back and report whether the text landed. Best effort -- contenteditable fields update their accessibility text lazily.' },
        verify_delay_ms: { type: 'integer', description: 'type only: settle before the verification read (default 500).' },
        ...
        state: { type: 'string', enum: ['exists', 'gone', 'foreground', 'visible', 'text_stable'], description: 'wait only, default exists. text_stable = return once the window text stops changing (and return it as text) -- use it to wait for a reply instead of guessing a sleep. Add absent/contains when a "still working" placeholder could sit still long enough to look finished.' },
        absent: { type: 'string', description: 'wait state:"text_stable" only: the settled text must NOT contain this. Guards against settling on a still-working placeholder (e.g. absent:"正在回应" on the ChatGPT desktop app).' },
        contains: { type: 'string', description: 'wait state:"text_stable" only: the settled text must contain this.' },
